...
2. **Shell config** (`~/.zshrc`) — _only for secret-style variables_ like `*_API_KEY`, `*_TOKEN`, `*_PAT`
3. **Global Configuration File** (`~/.context/.env`)
4. **Default Values** (lowest)

> **Shell config note:** Desktop apps and MCP hosts often start servers in non-interactive shells, so variables exported in `~/.zshrc` may not be inherited automatically. Claude Context will also **parse** `~/.zshrc` (it does not execute it) for simple `export NAME=value` assignments when looking up secret-style variables.
...
| `CONTEXT_ALLOW_DOC_EXTENSIONS` | Allow documentation/notebook extensions (`.md`, `.markdown`, `.ipynb`) when added via `CUSTOM_EXTENSIONS` or API config | `false` |
| `CUSTOM_IGNORE_PATTERNS`       | Additional ignore patterns (comma-separated, e.g., `temp/**,*.backup,private/**`)                                       | None    |

> **Docs/notebooks note:** `.md`, `.markdown`, and `.ipynb` are blocked by default (even if included in `CUSTOM_EXTENSIONS`) to avoid accidental embedding costs. To allow them intentionally, set `CONTEXT_ALLOW_DOC_EXTENSIONS=1`. You can still use `CUSTOM_IGNORE_PATTERNS` for fine-grained exclusions.
