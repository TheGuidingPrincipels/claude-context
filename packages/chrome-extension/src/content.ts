...
async function checkIndexStatus() {
  ...
  try {
    chrome.runtime.sendMessage({ action: 'checkIndexStatus', owner, repo }, (response) => {
...
function displayResults(results: any[]) {
  ...
  sortedResults.forEach((result) => {
    ...
    const matchPercentage =
      result.score !== undefined && result.score !== null ? result.score * 100 : 0;
    let matchClass = 'low';
      ...
      if (response && response.success) {
        ...
        if (statusEl) {
          statusEl.textContent =
            '✅ Index cleared. Repository needs to be indexed before searching';
          statusEl.style.color = '#22c55e';
        }
        ...
        if (statusEl) {
          statusEl.textContent =
            '❌ Failed to clear index: ' + (response?.error || 'Unknown error');
          statusEl.style.color = '#ef4444';
        }
      }
    });
  ...
  chrome.runtime.sendMessage({ action: 'getIndexedRepos' }, (response) => {
    ...
    if (response && response.success) {
      ...
      recentReposList.innerHTML =
        'Error loading recent repositories: ' + (response?.error || 'Unknown error');
    }
  });
}
...
function displayRecentRepos(repos: any[]) {
  ...
  if (!repos || repos.length === 0) {
    recentReposList.innerHTML =
      '<div class="color-fg-muted text-center py-3">No recently indexed repositories</div>';
    return;
  }
  ...
  repos.forEach((repo) => {
    ...
    const lastSearchDate = repo.lastSearchAt
      ? new Date(repo.lastSearchAt).toLocaleDateString()
      : 'Never';
