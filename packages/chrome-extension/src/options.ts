...
function saveOptions() {
  ...
  if (tokenInput) {
    ...
    addDebugInfo(
      `Saving settings: githubToken=${token ? '***' : 'empty'}, openaiToken=${openaiToken ? '***' : 'empty'}, milvusAddress=${milvusAddress}`
    );
    ...
    chrome.storage.sync.set(
      ...
      () => {
        ...
        chrome.storage.sync.get(['githubToken', 'openaiToken', 'milvusAddress'], (items) => {
          addDebugInfo(
            `Verified tokens saved: githubToken=${items.githubToken ? '***' : 'empty'}, openaiToken=${items.openaiToken ? '***' : 'empty'}, milvusAddress=${items.milvusAddress || 'empty'}`
          );
        });
      }
    );
...
function restoreOptions() {
  ...
  chrome.storage.sync.get(
    ...
    (items) => {
      ...
      addDebugInfo(
        `Restoring options: githubToken=${items.githubToken ? '***' : 'empty'}, openaiToken=${items.openaiToken ? '***' : 'empty'}, milvusAddress=${items.milvusAddress || 'empty'}`
      );
