...
class EmbeddingModel {
  ...
  static async getInstance(
    _progress_callback: Function | undefined = undefined
  ): Promise<(input: string | string[], options?: any) => Promise<{ data: number[] }>> {
  ...
  return new Promise((resolve, reject) => {
    chrome.storage.sync.get(['githubToken'], async (items) => {
      if (chrome.runtime.lastError) {
        ...
        reject(
          new Error(
            'GitHub token not found. Please configure your GitHub token in the extension settings.'
          )
        );
      } else {
        try {
          ...
          reject(
            new Error(
              `GitHub token validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
            )
          );
        }
      }
...
async function checkRepositoryAccess(owner: string, repo: string): Promise<void> {
  ...
  if (response.status === 404) {
    throw new Error(
      'Repository not found or you do not have access to it. Please check the repository name and ensure your GitHub token has the necessary permissions.'
    );
  }
  ...
  if (response.status === 403) {
    ...
    throw new Error(
      'Access forbidden. Your GitHub token may not have sufficient permissions to access this repository.'
    );
  }
...
async function fetchRepoFiles(owner: string, repo: string): Promise<any[]> {
  ...
  return data.tree.filter(
    (item: any) =>
      item.type === 'blob' &&
      item.path.match(
        /\.(ts|tsx|js|jsx|py|java|cpp|c|h|hpp|cs|go|rs|php|rb|swift|kt|scala|m|mm|md)$/
      )
  );
}
...
async function handleTestMilvusConnection(sendResponse: Function) {
  try {
    ...
    if (errorMessage.includes('fetch')) {
      errorMessage =
        'Network error: Unable to connect to Milvus server. Check address and network connectivity.';
...
async function processFileList(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (chunks.length > 50) {
        console.warn(
          `⚠️  File ${file.path} generated ${chunks.length} chunks (${Math.round(content.length / 1024)}KB)`
        );
      } else if (content.length > 100000) {
        console.log(
          `📄 Large file ${file.path}: ${Math.round(content.length / 1024)}KB -> ${chunks.length} chunks`
        );
      }
