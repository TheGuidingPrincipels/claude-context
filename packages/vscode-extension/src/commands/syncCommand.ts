...
export class SyncCommand {
  ...
  async execute(): Promise<void> {
    ...
    if (this.isSyncing) {
      vscode.window.showWarningMessage(
        'Sync is already in progress. Please wait for it to complete.'
      );
      return;
    }
    ...
    try {
      ...
      if (syncStats) {
        ...
        if (totalChanges > 0) {
          ...
          console.log(
            `[SYNC] Sync complete for '${codebasePath}'. Added: ${syncStats.added}, Removed: ${syncStats.removed}, Modified: ${syncStats.modified}`
          );
        } else {
  ...
  async executeSilent(): Promise<void> {
    ...
    try {
      ...
      if (totalChanges > 0) {
        console.log(
          `[AUTO-SYNC] Silent sync complete for '${codebasePath}'. Added: ${syncStats.added}, Removed: ${syncStats.removed}, Modified: ${syncStats.modified}`
        );
