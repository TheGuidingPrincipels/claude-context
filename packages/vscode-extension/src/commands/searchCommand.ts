...
export class SearchCommand {
  ...
  async execute(preSelectedText?: string): Promise<void> {
    ...
    try {
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          const extensionInput = await vscode.window.showInputBox({
            placeHolder:
              'Optional: filter by file extensions (e.g. .ts,.py,.java) – leave empty for all',
          ...
          if (fileExtensions.length > 0) {
            const invalid = fileExtensions.filter(
              (e) => !(e.startsWith('.') && e.length > 1 && !/\s/.test(e))
            );
            if (invalid.length > 0) {
              vscode.window.showErrorMessage(
                `Invalid extensions: ${invalid.join(', ')}. Use proper extensions like '.ts', '.py'.`
              );
              return;
            }
      ...
      vscode.window.showErrorMessage(
        `Search failed: ${error}. Please ensure the codebase is indexed.`
      );
    }
  }
  ...
  async executeForWebview(
    ...
    if (fileExtensions && fileExtensions.length > 0) {
      const invalid = fileExtensions.filter(
        (e) => !(typeof e === 'string' && e.startsWith('.') && e.length > 1 && !/\s/.test(e))
      );
      if (invalid.length > 0) {
        throw new Error(
          `Invalid extensions: ${invalid.join(', ')}. Use proper extensions like '.ts', '.py'.`
  ...
  private generateQuickPickItems(
    ...
    return results.slice(0, 20).map((result, index) => {
      ...
      const truncatedContent =
        result.content.length <= 150 ? result.content : result.content.substring(0, 150) + '...';
