  ...
  Ollama: {
    ...
    optionalFields: [
      ...
      },
      {
        name: 'keepAlive',
        type: 'string',
        description: 'Keep model alive duration',
        inputType: 'text',
        placeholder: '5m',
      },
    ] as FieldDefinition[],
  ...
  Gemini: {
    ...
    optionalFields: [
      ...
      },
      {
        ...
        type: 'number',
        description: 'Output dimension (supports Matryoshka representation)',
        inputType: 'text',
        placeholder: '3072',
      },
    ] as FieldDefinition[],
...
export class ConfigManager {
   ...
   */
  static getSupportedProviders(): Record<
    string,
    {
      name: string;
  ...
  > {
    ...
    for (const [providerKey, providerInfo] of Object.entries(EMBEDDING_PROVIDERS)) {
      ...
      const models =
        providerKey === 'Ollama' ? {} : (providerInfo.class as any).getSupportedModels();
  ...
  async saveSplitterConfig(splitterConfig: SplitterConfig): Promise<void> {
    ...
    await workspaceConfig.update(
      'splitter.type',
      splitterConfig.type || SplitterType.AST,
      vscode.ConfigurationTarget.Global
    );
    await workspaceConfig.update(
      'splitter.chunkSize',
