...
> 💡 **Where to put secrets:** The recommended place is `~/.context/.env`. If you keep API keys/tokens in `~/.zshrc`, note that MCP hosts often run non-interactively, so exports may not be inherited. Claude Context will also parse `~/.zshrc` for secret-style variables like `*_API_KEY`, `*_TOKEN`, `*_PAT`.
...
# Docs/notebooks extensions (.md,.markdown,.ipynb) are blocked by default to avoid accidental embedding costs.
# To allow them intentionally:
...
These settings work in combination with tool parameters - patterns from both sources will be merged together. Even when docs/notebooks extensions are allowed, use `CUSTOM_IGNORE_PATTERNS` for fine-grained exclusions.

## 🔒 Search-only Mode (Recommended)

Claude Context MCP supports two tool-surface modes:

- **Search-only** (`MCP_TOOL_MODE=search`, default): exposes only `search_code` + `get_indexing_status`
- **Admin** (`MCP_TOOL_MODE=admin`): _can_ expose `index_codebase` + `clear_index`, but only if the second latch `MCP_EXPOSE_ADMIN_TOOLS=true` is also set

This repo defaults to **search-only for agents** to prevent surprise embedding spend and accidental full re-indexing.

### Terminal/Admin Commands (No MCP Tools)

Indexing and maintenance are done via explicit terminal flags (these run even when `MCP_TOOL_MODE=search`):

```bash
# Full initial index (refuses if an index already exists)
node dist/index.js --admin-index /ABS/PATH/TO/CODEBASE

# Incremental sync (added + modified code, with caps)
node dist/index.js --admin-sync /ABS/PATH/TO/CODEBASE

# Incremental sync (bypass caps; explicit cost acceptance)
node dist/index.js --admin-sync-force /ABS/PATH/TO/CODEBASE

# Adopt/register an existing cloud index into the local snapshot (no embedding)
node dist/index.js --admin-adopt /ABS/PATH/TO/CODEBASE

# Delete chunks by file extension (no embedding; requires confirmation)
node dist/index.js --admin-purge-ext /ABS/PATH/TO/CODEBASE .md,.markdown,.ipynb

# Drop + rebuild (expensive; requires confirmation)
node dist/index.js --admin-reindex /ABS/PATH/TO/CODEBASE
```

### Incremental Daemon (Optional, Recommended)

Enable a singleton background daemon to keep indexes fresh by syncing **only changed code files**:

```bash
MCP_AUTOSTART_DAEMON=1
CONTEXT_DAEMON_INTERVAL_MINUTES=15
CONTEXT_DAEMON_CODEBASE_ALLOWLIST=/ABS/PATH/ONE,/ABS/PATH/TWO
CONTEXT_DAEMON_CODEBASE_BLOCKLIST=/ABS/PATH/TO/EXCLUDE

# Safety caps (default values shown)
CONTEXT_SYNC_MAX_EMBED_FILES=200
CONTEXT_SYNC_MAX_EMBED_BYTES=2000000
```

### Monitoring / Troubleshooting

- `~/.context/mcp-codebase-snapshot.json`: codebase registry + embedding metadata + **last full index** timestamps
- `~/.context/daemon-status.json`: daemon **incremental sync** timestamps, per-codebase stats, and last errors
- `~/.context/merkle/`: FileSynchronizer snapshots used for change detection
- `get_indexing_status`: shows snapshot status and, when available, daemon incremental sync recency/errors
...
> 🔒 **Admin-only:** This tool is only available when `MCP_TOOL_MODE=admin` **and** `MCP_EXPOSE_ADMIN_TOOLS=true`.
> In search-only mode, it is hidden and rejected. For day-to-day usage, prefer the terminal admin commands
> (`--admin-index`, `--admin-sync`, etc.) to avoid accidental embedding spend from agents.
