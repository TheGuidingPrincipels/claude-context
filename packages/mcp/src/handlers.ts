...
import { getSelfIndexScriptPath } from './daemon/admin-cli.js';
import {
  getDaemonStatusFilePath,
  type DaemonStatusFile,
  type CodebaseRunStatus,
} from './daemon/status.js';

function getAdminCommandPrefix(): string {
...
export class ToolHandlers {
  ...
  private lastCloudSyncTime: number = 0;
  private static CLOUD_SYNC_TTL_MS = 60000; // 60 seconds
  ...
  private isAllowedByAdminAllowlist(codebasePath: string): boolean {
    if (this.config.toolCodebaseAllowlist.length === 0) {
      return false;
    }
    return this.config.toolCodebaseAllowlist.includes(codebasePath);
  }

  private getEmbeddingConfigForCodebase(codebasePath: string): {
    embeddingProvider: ContextMcpConfig['embeddingProvider'];
    embeddingModel: string;
  } {
    const info = this.snapshotManager.getCodebaseInfo(codebasePath) as CodebaseInfo | undefined;
    // Only indexed codebases have embedding metadata
    const indexedInfo = info?.status === 'indexed' ? (info as CodebaseInfoIndexed) : undefined;
    const embeddingProvider = indexedInfo?.embeddingProvider ?? this.config.embeddingProvider;
    const embeddingModel = indexedInfo?.embeddingModel ?? this.config.embeddingModel;
    return { embeddingProvider, embeddingModel };
  }

  private getContextForCodebase(codebasePath: string): Context {
    const { embeddingProvider, embeddingModel } = this.getEmbeddingConfigForCodebase(codebasePath);
    ...
    const newContext = new Context({ embedding, vectorDatabase });
  ...
  private async syncIndexedCodebasesFromCloud(): Promise<void> {
    if (Date.now() - this.lastCloudSyncTime < ToolHandlers.CLOUD_SYNC_TTL_MS) {
      return;
    }
    ...
    try {
            ...
            if (metadataStr) {
              try {
                ...
                if (codebasePath && typeof codebasePath === 'string') {
                  const canonicalCloud = resolveCanonicalPath(codebasePath);
                  console.log(
                    `[SYNC-CLOUD] 📍 Found codebase path: ${codebasePath} (canonical: ${canonicalCloud}) in collection: ${collectionName}`
                  );
                  cloudCodebases.add(canonicalCloud);
                } else {
                  console.warn(
      ...
      // Remove local codebases that don't exist in cloud (compare canonical forms)
      for (const localCodebase of localCodebases) {
        const localCanonical = resolveCanonicalPath(localCodebase);
        console.log(
          `[SYNC-CLOUD] 🔄 Comparing local "${localCodebase}" (canonical: "${localCanonical}") against cloud`
        );
        if (!cloudCodebases.has(localCanonical)) {
  ...
  public async handleIndexCodebase(args: any) {
    // Defense-in-depth: block indexing unless server is explicitly in admin mode
    if (this.config.toolMode !== 'admin' || !this.config.exposeAdminTools) {
      return {
        content: [
          {
            type: 'text',
            text: 'Error: Indexing is disabled in this server. Only the singleton daemon can index.',
          },
        ],
        isError: true,
      };
    ...
    try {
      ...
      if (!this.isAllowedByAdminAllowlist(absolutePath)) {
        return {
          content: [
            {
              type: 'text',
              text:
                `Error: Admin indexing is restricted. '${absolutePath}' is not in MCP_CODEBASE_ALLOWLIST.\n\n` +
                `Set env MCP_CODEBASE_ALLOWLIST to a CSV list of absolute codebase roots you explicitly allow.`,
            },
          ],
          isError: true,
        };
      ...
      const snapshotSaysIndexed = this.snapshotManager.getIndexedCodebases().includes(absolutePath);
      const hasIndexInCloud = await this.context.hasIndex(absolutePath);

      // Check if the snapshot and cloud index are in sync
      if (snapshotSaysIndexed !== hasIndexInCloud) {
        console.warn(`[INDEX-VALIDATION] ❌ Snapshot and cloud index mismatch: ${absolutePath}`);
      }

      // Guardrail: refuse to index into an existing cloud collection unless force=true.
      // This prevents accidental full re-indexing that would duplicate embeddings and increase costs.
      if (hasIndexInCloud && !forceReindex) {
        return {
          content: [
            {
              type: 'text',
              text:
                `Error: A vector index already exists for '${absolutePath}'. Refusing to re-index to avoid duplicate embedding cost.\n\n` +
                `Use terminal admin commands instead:\n` +
                `- Sync incrementally: ${getAdminCommandPrefix()} --admin-sync '${absolutePath}'\n` +
                `- Register/adopt into snapshot (if search reports missing): ${getAdminCommandPrefix()} --admin-adopt '${absolutePath}'\n` +
                `- Force full rebuild (expensive): ${getAdminCommandPrefix()} --admin-reindex '${absolutePath}'`,
            },
          ],
          isError: true,
        };
      ...
      // If snapshot says indexed but cloud index is missing, refuse non-force indexing to avoid surprises.
      if (snapshotSaysIndexed && !hasIndexInCloud && !forceReindex) {
        return {
          content: [
            {
              type: 'text',
              text:
                `Error: Snapshot reports '${absolutePath}' as indexed, but no vector index was found in the database.\n\n` +
                `Refusing to proceed without explicit confirmation. Use one of:\n` +
                `- Force rebuild (expensive): ${getAdminCommandPrefix()} --admin-reindex '${absolutePath}'\n` +
                `- Or fix snapshot/state mismatch first.`,
            },
          ],
          isError: true,
        };
      ...
      if (currentStatus === 'indexfailed') {
        const failedInfo = this.snapshotManager.getCodebaseInfo(absolutePath) as
          | CodebaseInfoIndexFailed
          | undefined;
        console.log(
  ...
  private async startBackgroundIndexing(
    ...
    try {
      ...
      // Preserve embedding metadata in snapshot (used by daemon/search to avoid model mismatch)
      const updatedInfo = this.snapshotManager.getCodebaseInfo(absolutePath) as
        | CodebaseInfo
        | undefined;
      if (updatedInfo && updatedInfo.status === 'indexed') {
        const indexedInfo = updatedInfo as CodebaseInfoIndexed;
        indexedInfo.embeddingProvider =
          embeddingProvider.getProvider() as CodebaseInfoIndexed['embeddingProvider'];
        indexedInfo.embeddingModel = this.config.embeddingModel;
        indexedInfo.embeddingDimension = embeddingProvider.getDimension();
      }
  ...
  public async handleSearchCode(args: any) {
    ...
    try {
      ...
      if (!isIndexed && !isIndexing) {
        const indexingHint =
          this.config.toolMode === 'admin'
            ? `Please index it first using the index_codebase tool.`
            : `Indexing tools are disabled (MCP_TOOL_MODE=search). Index this codebase from the terminal (admin CLI) or via your indexer daemon.`;
        return {
          content: [
            {
              type: 'text',
              text: `Error: Codebase '${absolutePath}' is not indexed. ${indexingHint}`,
            },
          ],
          isError: true,
        };
      ...
      const contextForSearch = this.getContextForCodebase(absolutePath);

      // Log embedding provider information before search
      const embeddingProvider = contextForSearch.getEmbedding();
      console.log(
      ...
      const searchResults = await contextForSearch.semanticSearch(
        absolutePath,
        query,
      ...
      const looksLikeEmbeddingAuthError =
        lower.includes('status code: 401') ||
        lower.includes('provided api key is invalid') ||
        lower.includes('invalid api key') ||
        lower.includes('incorrect api key') ||
        (lower.includes('api key') && lower.includes('invalid')) ||
        (lower.includes('api key') && lower.includes('unauthorized'));

      const looksLikeMilvusError =
        lower.includes('milvus') ||
        lower.includes('failed to query milvus') ||
        lower.includes('failed to query collection') ||
        (lower.includes('status code: 401') && lower.includes('milvus')) ||
        (lower.includes('unauthorized') && lower.includes('milvus'));

      if (looksLikeEmbeddingAuthError && !looksLikeMilvusError) {
        let embeddingProvider = this.config.embeddingProvider;
        try {
          const abs = ensureAbsolutePath(codebasePath);
          embeddingProvider = this.getEmbeddingConfigForCodebase(abs).embeddingProvider;
        } catch {
          // Best-effort: keep default provider
        }

        switch (embeddingProvider) {
          case 'VoyageAI':
            hints.push(`🔐 Embedding auth failed. Check that VOYAGEAI_API_KEY is set and valid.`);
            break;
          case 'OpenAI':
            hints.push(`🔐 Embedding auth failed. Check that OPENAI_API_KEY is set and valid.`);
            break;
          case 'Gemini':
            hints.push(`🔐 Embedding auth failed. Check that GEMINI_API_KEY is set and valid.`);
            break;
          case 'Ollama':
            hints.push(
              `🔐 Embedding provider is Ollama. If embedding fails, check OLLAMA_HOST and that the Ollama server is reachable.`
            );
            break;
          default:
            hints.push(
              `🔐 Embedding auth failed. Check your embedding provider API key (e.g., VOYAGEAI_API_KEY / OPENAI_API_KEY / GEMINI_API_KEY).`
            );
        }
      }

      if (looksLikeMilvusError) {
        hints.push(
          `🗄️ Vector DB error. Check MILVUS_TOKEN and MILVUS_ADDRESS (and network access to Milvus/Zilliz).`
        );
      }
        ...
        content: [
          {
            type: 'text',
            text:
              `Error searching code: ${errorMessage}` +
              (hints.length > 0 ? `\n\n${hints.join('\n')}` : ''),
          },
        ],
        isError: true,
  ...
  public async handleClearIndex(args: any) {
    // Defense-in-depth: block index clearing unless server is explicitly in admin mode
    if (this.config.toolMode !== 'admin' || !this.config.exposeAdminTools) {
      return {
        content: [
          {
            type: 'text',
            text: 'Error: Index clearing is disabled in this server. Only admin mode can clear indexes.',
          },
        ],
        isError: true,
      };
          ...
          content: [
            {
              type: 'text',
              text:
                `Error: Admin index clearing is restricted. '${absolutePath}' is not in MCP_CODEBASE_ALLOWLIST.\n\n` +
                `Set env MCP_CODEBASE_ALLOWLIST to a CSV list of absolute codebase roots you explicitly allow.`,
            },
          ],
          isError: true,
  ...
  public async handleGetIndexingStatus(args: any) {
    ...
    try {
      ...
      switch (status) {
        case 'indexed':
          if (info && 'indexedFiles' in info) {
            ...
            statusMessage += `\n🕐 Last full index (snapshot): ${new Date(indexedInfo.lastUpdated).toLocaleString()}`;
          } else {
            statusMessage = `✅ Codebase '${absolutePath}' is fully indexed and ready for search.`;
          }
          break;
      ...
      // Daemon incremental sync information (optional)
      try {
        const daemonStatusPath = getDaemonStatusFilePath();
        if (fs.existsSync(daemonStatusPath)) {
          const raw = fs.readFileSync(daemonStatusPath, 'utf8');
          const daemonStatus = JSON.parse(raw) as DaemonStatusFile;
          const runStatus: CodebaseRunStatus | undefined = daemonStatus.codebases?.[absolutePath];

          if (runStatus) {
            const lastRunAt = runStatus.lastRunAt
              ? new Date(runStatus.lastRunAt).toLocaleString()
              : 'Unknown';
            statusMessage += `\n\n🔄 Daemon incremental sync`;
            statusMessage += `\n🕐 Last daemon run: ${lastRunAt}`;
            statusMessage += `\nΔ Changes: +${runStatus.added} -${runStatus.removed} ~${runStatus.modified}`;
            if (typeof runStatus.lastRunDurationMs === 'number') {
              statusMessage += `\n⏱ Duration: ${runStatus.lastRunDurationMs}ms`;
            }
            if (runStatus.lastError) {
              statusMessage += `\n⚠️ Last daemon error: ${runStatus.lastError}`;
            }
          } else {
            statusMessage +=
              `\n\nℹ️ Daemon status file found, but this codebase is not tracked by the daemon. ` +
              `Check CONTEXT_DAEMON_CODEBASE_ALLOWLIST.`;
          }
        }
      } catch (error) {
        console.warn('[STATUS] Failed to read daemon status file:', error);
      }
