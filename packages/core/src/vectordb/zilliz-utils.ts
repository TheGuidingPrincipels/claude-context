...
export class ClusterManager {
  ...
  constructor(config?: ZillizConfig) {
    ...
    this.baseUrl =
      envManager.get('ZILLIZ_BASE_URL') || config?.baseUrl || 'https://api.cloud.zilliz.com';
    this.token = envManager.get('MILVUS_TOKEN') || config?.token || '';

    if (!this.token) {
      throw new Error(
        'Zilliz API token is required. Please provide it via MILVUS_TOKEN environment variable or config parameter.'
      );
    }
  }
