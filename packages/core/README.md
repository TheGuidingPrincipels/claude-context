...
- `indexCodebase(path, progressCallback?, forceReindex?)` - Index an entire codebase (refuses to re-index into an existing collection unless `forceReindex=true`)
- `reindexByChange(path, progressCallback?, options?)` - Incrementally re-index only changed files (supports safety caps via `options`)
...
#### `ReindexByChangeOptions`

```typescript
type ReindexByChangeOptions = {
  maxEmbedFiles?: number; // default: 200
  maxEmbedBytes?: number; // default: 2_000_000 (2MB)
  includeModified?: boolean; // default: true
  bypassCaps?: boolean; // default: false
};
```
